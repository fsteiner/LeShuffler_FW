...
3. **Created Version D folder structure**:
   ...
   ├── Bootloader/         (modified from Version C)
   ├── Tools/              (Python tools)
   ├── Keys/test/          (test keys)
   └── Keys/production/    (empty - generate before release)
   ```
...
3. File → New → STM32 Project from Existing .ioc
...
- `uECC.c` → Core/Src/
- `uECC.h` → Core/Inc/
- `uECC_config.h` → Core/Inc/

**Step 5: Add crypto implementation**
- `crypto.c` → Core/Src/
- `crypto.h` → Core/Inc/
- `crypto_keys.h` → Core/Inc/
...
- STM32 NOR flash: can only change 1→0, not 0→1 without erase
...
├── v1_shipped/          # Original bootloader (shipped devices)
│   ├── bootload.c
│   ├── bootload.h
│   └── usbd_cdc_if.c
└── v2_robust/           # New bootloader with safe restart
    ├── bootload.c
    ├── bootload.h
    └── usbd_cdc_if.c
```
...
- If disconnect happens mid-transfer → incomplete firmware with valid header
- Bootloader jumps to incomplete app → device stuck
...
User presses "Firmware Update"
    ↓
Bootloader starts → FLASH ERASED IMMEDIATELY
    ↓
3 short beeps + 1 long beep (USB ready)
    ↓
User runs updater → connects → transfer begins
    ↓
...
Verifying USB stability (5 tests)...
  ...
  ...
```
If any test fails → warns user USB is unstable (but must still complete update somehow).
...
**Tested 2025-01-02:** Windows + v1.0 with all safety measures → SUCCESS
...
├── LeShuffler_Updater.exe    # The packaged executable
├── LeShuffler.bin            # The firmware file (you provide this)
...
├── LeShuffler_STLink_Flasher.exe      (8 MB)
├── Bootloader_E.bin                   (48 KB) -> 0x08000000
├── LeShuffler.bin                     (218 KB) -> 0x08020000
└── README.txt
```
...
├── Bootloader/                     # Bootloader v2.1 project
│   ├── Core/Src/main.c            # Fixed erase logic
│   ├── Core/Src/bootload.c        # RESUME packet support
│   └── ...
├── Tools/
│   ├── firmware_updater_v5_1.py   # USB updater with v2.1 support
...
3. Device returns to application ✓
...
### Notes for Claude (Working Practices)
...
**STM32_Programmer_CLI on Windows (PowerShell)**:
```powershell
& "C:\Program Files\STMicroelectronics\STM32Cube\STM32CubeProgrammer\bin\STM32_Programmer_CLI.exe" -c port=SWD -ob displ
```

**RDP Level 1 Important Notes (STM32H7)**:
- **Mac ST-LINK drivers don't work with RDP Level 1** - always use Windows for ST-LINK operations after RDP is set
- **CLI works for**: Flashing, setting RDP 0→1, mass erase at RDP 0
- **GUI required for recovery**: Mass erase or RDP 1→0 (CLI doesn't work reliably)
- **To recover from RDP Level 1**: Open GUI → OB tab → Set RDP to 0xAA → Apply (triggers mass erase)
- **Power cycle required** after setting RDP for changes to take effect
- **Factory command**: `python stlink_flasher.py --rdp 1 -y` then power cycle

**Git Safety - CRITICAL**:
- **NEVER use `git checkout <file>`** to undo formatting changes - it reverts ALL changes including user's work
- **Use `git stash` first** if you need to temporarily undo changes: `git stash` → fix → `git stash pop`
- **After amending commits**, ALWAYS verify changes with `git show --stat` or `git diff HEAD~1`
- **Before any git revert/checkout**, check `git diff <file>` to see what will be lost
- **Verify code after git operations** by grepping for key function/variable names

---

### 2025-01-04: Session 7 - Bootloader Version Readable from Firmware

#### Problem:
Application firmware had no way to read the bootloader version. The version was only available via USB STATUS packet query.

#### Solution:
Store bootloader version at a fixed flash address (0x0800BFF0) that the application can read directly.

#### Implementation:

**1. Bootloader Linker File** (`Bootloader/STM32H733VGTX_BOOTLOADER.ld`):
```ld
.bootloader_version 0x0800BFF0 :
{
    KEEP(*(.bootloader_version))
} >FLASH
```

**2. Bootloader Version Struct** (`Bootloader/Core/Src/bootload.c`):
```c
__attribute__((section(".bootloader_version"), used))
const struct {
    uint16_t version;      // 0x0201 = v2.1
    uint16_t reserved;
    uint32_t magic;        // "BLVR" (0x424C5652)
    uint32_t build_date;
    uint32_t checksum;
...
#### Architecture:
```
Developer: encrypt_firmware.py → .sfu file (signed + encrypted)
User: firmware_updater_E.py → USB transfer
Device: Bootloader_E → verify signature, decrypt, flash
```
...
**Menu Location:** MAINTENANCE → (long press) → Technical Support Only → Test Watchdog
...
**Watchdog Reset Test:**
1. Navigate to: MAINTENANCE → (long press ENTER) → Technical Support Only
...
```
FLASH: 48K → 128K  (full sector 0)
Version address: 0x0800BFF0 → 0x0801FFF0
```
...
|----------|--------|-------|
| v3.0 bootloader + plain .bin | ✅ Yes | Original START/DATA/END still work |
| v3.0 bootloader + encrypted .sfu | ✅ Yes | New ENC_* packet types |
| Old app reading v3.0 version | ⚠️ Partial | Gets 0 (old address has code/garbage) |
...
7. If valid → FW_COMPLETE; if invalid → FW_ERROR
```
...
- Sends encrypted packets: ENC_START → ENC_DATA → ENC_END
...
**First attempt:**
- ✅ Bootloader v3.0 detected
- ✅ Header validated, flash erased
- ✅ Started sending 877 encrypted packets
- ❌ USB disconnected mid-transfer
- ❌ On reconnect: "ENC_START failed: Header validation failed"
...
**Developer (you):**
```
LeShuffler.bin → encrypt_firmware.py → LeShuffler.sfu
                 (with production keys)
```

**End User:**
```
LeShuffler.sfu → firmware_updater_encrypted.py → Device
...
- First ENC_DATA packet → immediate USB disconnect
...
- If Windows works → macOS USB workaround needed
- If Windows fails → debug the ENC_DATA crypto/flash path
...
|------|----------|--------|
| 1 | Validation + CRC only | ✅ 100% transferred (with reconnects) |
| 2 | + Hash update (`Crypto_SHA256_Update`) | 🔄 Testing on Windows |
| 3 | + Decrypt | Pending |
...
#### Key Finding:
- ✅ 100% transfer completed (no USB disconnects on Windows!)
- ✅ All 877 packets transferred successfully
- ✅ All CRC checks passed
- ❌ Signature verification failed at ENC_END
...
- **DANGER**: If bootloader has a critical bug → device is permanently bricked
...
| 2 | 0xCC | `RDP=0xCC` (⚠️ IRREVERSIBLE) |
...
| Action | Status |
|--------|--------|
| Read flash via debugger (ST-LINK) | ❌ Blocked |
| Read RAM via debugger | ❌ Blocked |
| Read option bytes via debugger | ❌ Blocked |
| JTAG/SWD memory access | ❌ Blocked |

**What's ALLOWED (normal operation):**
| Action | Status |
|--------|--------|
| CPU executes code normally | ✅ Works |
| Bootloader runs | ✅ Works |
| USB CDC works | ✅ Works |
| OTA updates via bootloader | ✅ Works |
| Flash write from running code (HAL_FLASH_Program) | ✅ Works |
| Connect debugger (but can't read) | ✅ Connects |
| Mass erase via debugger | ✅ Works (reverts to RDP 0) |
...
|----------|-------|-------|-------|
| Attacker reads flash | ✅ Can read | ❌ Can't read | ❌ Can't read |
...
1. **memcpy instead of decrypt** → ✅ No crash, transfer completes
2. **HAL_CRYP_Init alone** → ✅ Works (key loads OK)
3. **HAL_CRYP_Decrypt after Init** → ❌ Hard fault
4. **HAL_CRYP_SetConfig + Decrypt** → ✅ No crash but wrong output
...
#### What's Working:
- ✅ ENC_START (header validation, flash erase)
- ✅ ENC_DATA (CRC check, hash update, transfer)
- ✅ ENC_END (hash finalize, ECDSA signature verification)
- ✅ Transfer completes 100% on Windows
- ✅ Signature verification passes

#### What's NOT Working:
- ❌ AES-256-CBC decryption crashes or produces garbage
- ❌ Device stays in bootloader (invalid application in flash)
...
- ❌ Device unresponsive after update (decryption produced garbage)
...
- ✅ **SUCCESS!** Encrypted firmware update complete
- ✅ Device boots into decrypted firmware
- ✅ All 877 packets transferred and decrypted correctly
...
**Problem:** When device powered only by laptop USB (insufficient power for stepper motor), `carousel_init()` → `tmc2209_init()` hangs indefinitely, triggering watchdog reset. Screen stays on last animation frame.
