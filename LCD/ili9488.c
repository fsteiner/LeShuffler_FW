...
void 	 ili9488_DrawRGBImage8bit(uint16_t Xpos, uint16_t Ypos, uint16_t Xsize, uint16_t Ysize, uint8_t *pData);
...
#define ILI9488_DGCTR1         0xE2
#define ILI9488_DGCTR2         0xE3
...
void ili9488_Init(void)
{
  ...
  LCD_Delay(5);
  LCD_Delay(20);
  LCD_IO_WriteCmd8(ILI9488_INVOFF);	// COLOUR INVERSION (OFF WITH NEW LCD)
   LCD_Delay(120);
...
//-----------------------------------------------------------------------------
/**
  * @brief  Displays 8bit/pixel picture..
  * @param  pdata: picture address.
  * @param  Xpos:  Image X position in the LCD
  * @param  Ypos:  Image Y position in the LCD
  * @param  Xsize: Image X size in the LCD
...
void ili9488_DrawRGBImage8bit(uint16_t Xpos, uint16_t Ypos, uint16_t Xsize, uint16_t Ysize, uint8_t *pData)
{
	uint32_t size;
	  ...
	  #elif ILI9488_INTERFACE == 1
	  LCD_IO_WriteCmd8MultipleData8(ILI9488_RAMWR, pData, size * 2);
	  #endif
	  ILI9488_LCDMUTEX_POP();
}
