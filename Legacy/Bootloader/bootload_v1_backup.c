...
int32_t ProcessFirmwarePacket(FirmwarePacket_t *packet)
{
    ...
    switch (packet->packet_type) {
        case PACKET_TYPE_START:
            ...
            // Note: Flash unlock/lock is now done per-packet in DATA handler
            // This ensures flash is never left unlocked if USB disconnects

            // Only erase if not already erased (bootloader erases before USB init
            // when entering from app, so last_erased_sector will already be set)
            if (last_erased_sector == 0) {
        ...
        case PACKET_TYPE_DATA:
            ...
            // Small delay for flash settling
            HAL_Delay(5);
